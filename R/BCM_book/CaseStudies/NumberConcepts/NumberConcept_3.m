...
datastruct = struct('ns',ns,'fnq',fnq,'fn',fn,'fa',fam,'fq',fqm,'nz',nz,'gnq',gnq,'gn',gn,'ga',ga,'gq',gq);
...
if ~run_model
    load NumberConcept_3 samples stats
else
    if ~sampler
        ...
        [samples, stats, structarray] = matbugs(datastruct, ...
            ...
            'monitorParams', {'predga','predfaz','predgaz','predpi','predfa','predfpi','fv','gv','z'}, ...
        ...
        [samples, stats] = matjags( ...
            ...
            'nsamples', nsamples, ...
            'thin', nthin, ...
            'monitorparams', {'predga','predgaz','predfaz','predpi','predfa','predfpi','fv','gv','z'},...
            'savejagsoutput' , 1 , ...
            'verbosity' , 2 , ...
            'cleanup' , 0 , ...
            'workingdir' , 'tmpjags' );
    ...
    save NumberConcept_3 samples stats
end;
...
bins=[1:fn];
count=hist(samples.predfpi,bins);
...
for z=1:nz
    ...
    for i=1:gn
        count=hist(squeeze(samples.predgaz(:,:,z,i)),bins);
...
figure(4);clf;
set(gcf,'units','norm','pos',[.1 .1 .8 .8],'paperpositionmode','auto');
bins=[1:fn];threshold=0;
sc=9;sc2=1;sc3=2;
eps=.000;
mv=zeros(ns,1);
tau=1/6;
for i=1:ns
    % Use Lowest Above Some Proportion Tau
    count=hist(squeeze(samples.z(:,:,i)),[1:nz]);
    count=count/sum(count);
    match=find(count>=tau);
    mv(i)=min(match);
end;
% Posteriore Predictive Shaded Underlay
cm=-.5;cb=.99;
for z=1:nz
    subplot(2,3,z);hold on;
    for i=1:gn
        count=hist(squeeze(samples.predfaz(:,:,z,i)),bins);
        count=count/max(count);
        for j=1:fn
